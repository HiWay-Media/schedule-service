        ...
        for (let i = 0; i < assets.length; i++) {
            ...
            if (asset) {
                ...
                if (feed.shouldInsertLive) {
                    console.log(`[${feed.channelId}]: Adding schedule event (${ScheduleEventType.LIVE}): url=${feed.liveUrl}, start=${new Date(asset.start_time).toISOString()}, end=${new Date(asset.end_time).toISOString()}`);
                    ...
                    console.log(`[${feed.channelId}]: Adding schedule event (${ScheduleEventType.VOD}): title=${asset.title}, start=${new Date(asset.start_time).toISOString()}, end=${new Date(asset.end_time).toISOString()}`);
                    scheduleEventsToAdd.push(new ScheduleEvent({
                        id: uuidv4(),
                        channelId: feed.channelId,
                        title: asset.title,
